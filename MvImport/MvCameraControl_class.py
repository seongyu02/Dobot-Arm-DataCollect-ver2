...
def check_sys_and_update_dll():
    ...
    if currentsystem == 'Windows':
        ...
        # HIKRobot MVS Runtime 경로 자동 탐색(환경변수 + 기본 설치 경로)
        env_runtime_dirs = []
        for key in ("MVS_RUNTIME_DIR", "MVS_HOME", "MVSDK_PATH"):
            value = os.getenv(key, "")
            if value:
                env_runtime_dirs.extend([
                    value,
                    os.path.join(value, "Runtime", "Win64_x64"),
        ...
        checked_dirs = []
        for runtime_dir in env_runtime_dirs + default_runtime_dirs:
            if runtime_dir and runtime_dir not in checked_dirs:
                checked_dirs.append(runtime_dir)
        ...
        try:
            ...
            checked_text = "\n".join(checked_dirs) if checked_dirs else "(none)"
            raise FileNotFoundError(
                "Could not load MvCameraControl.dll. "
                "Install HIKRobot MVS Runtime and/or set one of "
                "MVS_RUNTIME_DIR, MVS_HOME, MVSDK_PATH.\n"
